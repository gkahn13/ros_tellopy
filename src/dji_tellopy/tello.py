...
class Tello:
    ...
    def __init__(self, local_ip, local_port, imperial=False, command_timeout=.3, tello_ip='192.168.10.1',
        ...
        self.socket_state = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)  # socket for sending cmd
    ...
    def _receive_state_thread(self):
        ...
        while True:
            try:
                self.response_state, ip = self.socket_state.recvfrom(3000)
                #print(self.response)
            except socket.error as exc:
                print ("Caught exception socket.error : %s" % exc)
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        # rate.sleep()
        # print('height: {0}'.format(tello.get_height()))
        # print('battery: {0}'.format(tello.get_battery()))
        # print('speed: {0}'.format(tello.get_speed()))

        # print(tello.send_command('tof?')) # good!
        # print(tello.send_command('baro?'))  # good!
        # print(tello.send_command('acceleration?')) # good!

        # print(tello.response_state)
        # start = time.time()
        # print(tello.socket_state.recvfrom(3000))
