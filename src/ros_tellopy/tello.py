...
from ros_tellopy.decorators import accepts
...
class Tello:
    ...
    LOGGER = logging.getLogger('dji_tellopy')
    LOGGER.addHandler(HANDLER)
    LOGGER.setLevel(logging.INFO)
    # use logging.getLogger('dji_tellopy').setLevel(logging.<LEVEL>) in YOUR CODE
        ...
        while True:
            try:
                ...
                if self.state_callback_fn is not None:
                    self.state_callback_fn(self.response_state)
            except Exception as e:
                self.LOGGER.error(e)
                break
    ...
    def send_rc_control(self, left_right_velocity, forward_backward_velocity, up_down_velocity, yaw_velocity):
        ...
        Returns:
            bool: True for successful, False for unsuccessful
        """
        if int(time.time() * 1000) - self.last_rc_control_sent < self.TIME_BTW_RC_CONTROL_COMMANDS: # * 1000 ???
            pass
        else:
