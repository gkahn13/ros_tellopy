...
directory = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../bags')
...
class JoyTeleopNode(object):
    ...
    def __init__(self):
        ...
        self._cmd_pub = rospy.Publisher(drone_type+'cmd_vel', Twist, queue_size=1)
        ...
        self._yaw_noise = StepNoise(**{'reset_step_range' : [5, 15], 'step_range' : [-0.05, 0.05], 'range' : [-1e-6, 1e-6]}) #[-0.5, 0.5]
        self._x_noise = StepNoise(**{'reset_step_range' : [50, 51], 'step_range' : [-0.005, 0.005], 'range' : [-1e-6, 1e-6]}) #[0.04, 0.041]
        
        self._ros_msgs = dict()
        self._ros_msg_times = dict()
        if collect_data:
            datetime.now()
            self.bag = rosbag.Bag(os.path.join(directory, drone_type_param+"_"+datetime.now().strftime("%m_%d_%H_%M_%S")+".bag"), 'w')
            if drone_type_param == "ardrone":
                self._ros_topics_and_types = dict([('/ardrone/odometry', Odometry)])
            else:
                self._ros_topics_and_types = dict([('/bebop/odom', Odometry)])
            for topic in self._ros_topics_and_types.keys():
                self._ros_msgs[topic] = None
            for topic, type in self._ros_topics_and_types.items():
                rospy.Subscriber(topic, type, self.ros_msg_update, (topic,))

            while not rospy.is_shutdown() and not any([v is None for v in self._ros_msgs.values()]):
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if rospy.get_time()>self._next_update_time or (not collect_data):
                ...
                cmd = Twist()
                cmd.linear.y = 0.0 # self._joy_axes[JoyTeleopNode.RIGHT_AXIS_X]
                cmd.linear.z = 0.2 * self._joy_axes[JoyTeleopNode.LEFT_AXIS_Y]
                if collect_data:
                    cmd.linear.x = np.clip(0.3 * self._joy_axes[JoyTeleopNode.RIGHT_AXIS_Y] + self._x_noise.step(), 0, 1)
                    cmd.angular.z = np.clip(1.0 * self._joy_axes[JoyTeleopNode.LEFT_AXIS_X] + self._yaw_noise.step(), -1, 1)
                else:
                ...
                if collect_data:
                    ...
                    for topic in self._ros_msgs.keys():
